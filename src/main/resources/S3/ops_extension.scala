/** Get a pre-signed URL for a given operation name.
  ...
  * @param params
  *   Parameters to pass to the operation. See the given operation for the expected operation parameters. In addition,
  *   you can also pass the "Expires" parameter to inform S3 how long the URL should work for.
  * @param expires
...
/** Uploads an arbitrarily sized buffer, blob, or stream, using intelligent concurrent handling of parts if the payload
  * is large enough. Note that this is the only operation for which the SDK can retry requests with stream bodies.
  * @return
...
/** Uploads an arbitrarily sized buffer, blob, or stream, using intelligent concurrent handling of parts if the payload
  * is large enough. You can configure the concurrent queue size by setting options. Note that this is the only
  * operation for which the SDK can retry requests with stream bodies.
  * @return
  *   The managed upload object that can call send() or track progress.
  */
...
/** Uploads an arbitrarily sized buffer, blob, or stream, using intelligent concurrent handling of parts if the payload
  ...
  *   The response data from the successful upload
  */
def uploadFuture(params: PutObjectRequest,
                 options: managedupload.ManagedUploadOptions
): Future[managedupload.SendData] = {
